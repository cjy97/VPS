...
# 自实现的训练DataSet类，用于加载划分后的patch图像
class My_Dataset(Dataset):
	def __init__(self, root_dir, patch_size=128):
		self.patch_size = patch_size

		self.clean_imgs = glob.glob(root_dir + "/src*")	# 存储数据集中所有src图像的路径
		self.clean_imgs.sort()

	def __len__(self):
		l = len(self.clean_imgs)
		return l

	def __getitem__(self, idx):
		# clean_fn = random.choice(self.clean_imgs)	# 从所有src图像中随机抽样
		clean_fn = self.clean_imgs[idx]

		clean_img = read_img(clean_fn)
		noise_img = read_img(clean_fn.replace('src', 'noise'))

		clean_img = np.mean(clean_img, axis=2)[:,:,np.newaxis]	# 转成[h, w, 1]的单通道灰度图
		noise_img = np.mean(noise_img, axis=2)[:,:,np.newaxis]

		# if self.patch_size > 0:
		# 	[clean_img, noise_img] = get_patch([clean_img, noise_img], self.patch_size)	# 从clean-noise图像上随机裁剪出patch_size大小的对应的patches
...
class Real(Dataset):
	def __init__(self, root_dir, sample_num, patch_size=128):
		...
		folders = glob.glob(root_dir + '/*')	# 训练图像目录下的所有子目录
		folders.sort()
		...
		for ind, folder in enumerate(folders):
			clean_imgs = glob.glob(folder + '/*GT_SRGB*')	# 每个子目录下的所有图像名
			clean_imgs.sort()
			print("clean_imgs: ", len(clean_imgs))

			for clean_img in clean_imgs:
				self.clean_fns[ind % sample_num].append(clean_img)
		
		print("self.clean_fns: ", self.clean_fns)
